...
def password_reset(request):
    ...
    if request.method=='GET':
        ...
        if form.is_valid():
            ...
            context['domain_override']=settings.SERVER_NAME
            form.save(**context)
...
class HomeView(LoginRequiredMixin,generic.ListView):
    ...
    def get_context_data(self,**kwargs):
        ...
        reserved_book_isbncodes=self.request.user.reserved_books.all().values_list('ISBNcode', flat=True)
        finished_book_isbncodes=self.request.user.finished_books.all().values_list('ISBNcode', flat=True)

        context['liked_list']=liked_list
        context['selected_page']=self.request.path
        context['reserved_book_isbncodes']=reserved_book_isbncodes
        context['finished_book_isbncodes']=finished_book_isbncodes
        return context
...
def create_book_model(ISBNcode):
...
@login_required
def register_reserved_book(request):
    if request.method == "POST":
        ISBNcode=json.loads(request.body).get('ISBNcode')
        book=BookData.objects.filter(ISBNcode=ISBNcode)
        if book.exists():
            book=book[0]
        else:
            book=create_book_model(ISBNcode)

        reserve_registration=ReserveRegistration.objects.filter(user=request.user,book=book)
        if reserve_registration.exists():
            reserve_registration.delete()
            reserved=False
        else:
            reserve_registration.create(user=request.user,book=book)
            reserved=True
        context={
            'ISBNcode':ISBNcode,
            'reserved':reserved
...
@login_required
def register_finished_book(request):
    if request.method == "POST":
        ISBNcode=json.loads(request.body).get('ISBNcode')
        book=BookData.objects.filter(ISBNcode=ISBNcode)
        if book.exists():
        ...
        finish_registration=FinishRegistration.objects.filter(user=request.user,book=book)
        if finish_registration.exists():
            finish_registration.delete()
            finished=False
        else:
            finish_registration.create(user=request.user,book=book)
            finished=True
        context={
            'ISBNcode':ISBNcode,
            'finished':finished
...
class UserPageAtHomeView(LoginRequiredMixin,generic.ListView):
    model=TweetModel
    ...
    def get_context_data(self,**kwargs):
        ...
        _context={
            ...
            'followed':followed,
            'selected_page':'/user-page-at-home/',
            'reserved_book_isbncodes':reserved_book_isbncodes,
            'finished_book_isbncodes':finished_book_isbncodes,
        }
        context={**_context, **context}

        return context

class UserPageAtReservedBookView(LoginRequiredMixin,generic.ListView):
    model=ReserveRegistration
    context_object_name = 'reserved_book_list'
    template_name='user_page.html'

    def get(self, *args, **kwargs):
        selected_user_id=self.kwargs['pk']
        if self.request.user.id != selected_user_id:
            return redirect(reverse('UserPageAtHome', args=[selected_user_id]))
        return super(UserPageAtReservedBookView, self).get(*args, **kwargs)

    def get_queryset(self,**kwargs):
        queryset = super().get_queryset(**kwargs)
        selected_user = CustomUser.objects.get(id=self.kwargs['pk'])
        queryset=queryset.filter(user=selected_user).order_by('-registered_at')
        return queryset
    ...
    def get_context_data(self,**kwargs):
        ...
        reserved_book_list=[]
        for _book in context['reserved_book_list']:
            reserved_book_list.append(_book.book)
        context['reserved_book_list']=reserved_book_list
        ...
        _context={
            ...
            'selected_user_followed_count':selected_user_followed_count,
            'selected_user':selected_user,
            'selected_page':'/user-page-at-reserved-book/',
            'reserved_book_isbncodes':reserved_book_isbncodes,
            'finished_book_isbncodes':finished_book_isbncodes,
        }
        context={**_context, **context}
...
class UserPageAtFinishedBookView(LoginRequiredMixin,generic.ListView):
    model=FinishRegistration
    ...
    def get(self, *args, **kwargs):
        ...
        if self.request.user.id != selected_user_id:
            return redirect(reverse('UserPageAtHome', args=[selected_user_id]))
        return super(UserPageAtFinishedBookView, self).get(*args, **kwargs)
    ...
    def get_context_data(self,**kwargs):
        ...
        finished_book_list=[]
        for _book in context['finished_book_list']:
            finished_book_list.append(_book.book)
        context['finished_book_list']=finished_book_list

        _context={
            'selected_user_following_count':selected_user_following_count,
            'selected_user_followed_count':selected_user_followed_count,
            'selected_user':selected_user,
            'selected_page':'/user-page-at-finished-book/',
        }
        context={**_context, **context}
...
def create_comment(request, tweet_pk):
    if request.method=="GET":
        ...
        reserved_book_isbncodes=request.user.reserved_books.all().values_list('ISBNcode', flat=True)
        finished_book_isbncodes=request.user.finished_books.all().values_list('ISBNcode', flat=True)
